...
// enums work more like const than types
enum card_suit : uint8_t { SPD, HRT, DIA, CLB };
enum card_rank : uint8_t { ACE = 1, DEUCE = 2, JACK = 11, QUEEN, KING };

// constexpr helps with the preprocessor process because it tells the processor that it can be computed during compile time
...
int main() {
    ...
    printf("sizeofdeck: %ld\n", sizeof(deck));
    printf("sizeofcards: %ld\n", sizeof(card));
